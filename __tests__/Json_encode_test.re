open Jest;
open Expect;
open! Json.Encode;

let _ = {
  test("null", () =>
    expect(null) |> toEqual @@ Obj.magic(Js.null)
  );

  test("string", () =>
    expect @@ string("foo") |> toEqual @@ Obj.magic("foo")
  );

  test("float", () =>
    expect @@ float(1.23) |> toEqual @@ Obj.magic(1.23)
  );

  test("int", () =>
    expect @@ int(23) |> toEqual @@ Obj.magic(23)
  );

  test("bool", () =>
    expect @@ bool(true) |> toEqual @@ Obj.magic(true)
  );

  test("date", () =>
    expect @@
    date(Js.Date.fromString("2012-04-23T18:25:43.511Z"))
    |> toEqual @@
    Obj.magic("2012-04-23T18:25:43.511Z")
  );

  test("char", () =>
    expect @@ char('a') |> toEqual @@ Obj.magic("a")
  );

  test("jsonDict - empty", () =>
    expect @@
    jsonDict @@
    Js.Dict.empty()
    |> toEqual @@
    Obj.magic @@
    Js.Dict.empty()
  );

  test("jsonDict - simple", () =>
    let o = Js.Dict.empty();
    Js.Dict.set(o, "x", int(42));

    expect @@ jsonDict(o) |> toEqual @@ Obj.magic(o);
  );

  test("dict - simple", () =>
    let o = Js.Dict.empty();
    Js.Dict.set(o, "x", 42);

    expect @@ dict(int, o) |> toEqual @@ Obj.magic(o);
  );

  test("object_ - empty", () =>
    expect @@ object_ @@ [] |> toEqual @@ Obj.magic @@ Js.Dict.empty()
  );

  test("object_ - simple", () =>
    expect @@
    object_([("x", int(42))])
    |> toEqual @@
    Obj.magic(Js.Dict.fromList([("x", 42)]))
  );

  test("array int", () =>
    expect @@ array(int, [|1, 2, 3|]) |> toEqual @@ Obj.magic([|1, 2, 3|])
  );

  test("list int", () =>
    expect @@ list(int, [1, 2, 3]) |> toEqual @@ Obj.magic([|1, 2, 3|])
  );

  test("jsonArray int", () =>
    expect @@
    jsonArray([|1, 2, 3|] |> Array.map(int))
    |> toEqual @@
    Obj.magic([|1, 2, 3|])
  );

  test("stringArray", () =>
    expect @@ stringArray([|"a", "b"|]) |> toEqual @@ Obj.magic([|"a", "b"|])
  );

  test("numberArray", () =>
    expect @@ numberArray([|0., 4.|]) |> toEqual @@ Obj.magic([|0, 4|])
  );

  test("boolArray", () =>
    expect @@
    boolArray([|true, false|])
    |> toEqual @@
    Obj.magic([|true, false|])
  );

  test("nullable (None)", () =>
    expect @@ nullable(string, None) |> toEqual @@ null
  );

  test("nullable (Some)", () =>
    expect @@
    nullable(string, Some("success"))
    |> toEqual @@
    string("success")
  );

  test("withDefault (None)", () =>
    expect @@
    withDefault(string("default"), string, None)
    |> toEqual @@
    string("default")
  );

  test("withDefault (Some)", () =>
    expect @@
    withDefault(string("default"), string, Some("success"))
    |> toEqual @@
    string("success")
  );

  test("pair", () =>
    expect @@
    pair(string, float, ("hello", 1.2))
    |> toEqual @@
    jsonArray([|string("hello"), float(1.2)|])
  );

  test("tuple2", () =>
    expect @@
    tuple2(string, float, ("hello", 1.2))
    |> toEqual @@
    jsonArray([|string("hello"), float(1.2)|])
  );

  test("tuple3", () =>
    expect @@
    tuple3(string, float, int, ("hello", 1.2, 4))
    |> toEqual @@
    jsonArray([|string("hello"), float(1.2), int(4)|])
  );

  test("tuple4", () =>
    expect @@
    tuple4(string, float, int, bool, ("hello", 1.2, 4, true))
    |> toEqual @@
    jsonArray([|string("hello"), float(1.2), int(4), bool(true)|])
  );
};
